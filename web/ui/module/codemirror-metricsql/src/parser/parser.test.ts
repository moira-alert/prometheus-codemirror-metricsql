...
describe('promql operations', () => {
  const testCases = [
    ...
    {
      expr: 'bottomk_avg(3, sum(process_resident_memory_bytes) by (job), "job=other")',
      expectedValueType: ValueType.vector,
      expectedDiag: [],
    },
    {
      expr: 'limitk(3, sum(process_resident_memory_bytes)) by (job)',
      expectedValueType: ValueType.vector,
      expectedDiag: [],
    },
    {
      expr: 'quantiles("phiLabel", 0.5, 0.95, 0.65, process_resident_memory_bytes)',
      expectedValueType: ValueType.vector,
      expectedDiag: [],
    },
  ];
